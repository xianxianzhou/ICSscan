...
@Slf4j
@Component
public class JunoTask extends DataAbstract {
...
//    @Scheduled(cron = "*/2 * * * * ?")
    public void getBlockInfo() {
        ...
        try {
            ChainInfo chainInfo = chainInfoService.getByChainid(nodeConfiguration.getJunoChainid());
            if (chainInfo == null) {
                return;
            }
            Long height = chainInfo.getHeight();
            String url = nodeConfiguration.getJunoApi() + String.format(getBlockByHeight, height);
           log.info("chain id: {} get block height: {}",getChainId(),height);
            ...
            if (resultStr != null) {
                ...
                if (blockResult != null && blockResult.getCode() == 0) {
                            ...
                            public void run() {
                                ...
                                for (String tx : list) {
                                    try {
                                        ...
                                        String transactionUrl = nodeConfiguration.getJunoApi() + String.format(txs, hash);
                    ...
                    height++;
                    chainInfoService.updateHeight(nodeConfiguration.getJunoChainid(), height);
    ...
    public String getChainId() {
        return nodeConfiguration.getJunoChainid();
    }
}
