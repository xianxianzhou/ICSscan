...
@Slf4j
@Component
public class OmniflixhubTask extends DataAbstract {
...
//    @Scheduled(cron = "*/2 * * * * ?")
    public void getBlockInfo() {
        try {
            ChainInfo chainInfo = chainInfoService.getByChainid(nodeConfiguration.getOmniflixChainid());
            if (chainInfo == null) {
                return;
            }
            Long height = chainInfo.getHeight();
            String url = nodeConfiguration.getOmniflixApi() + String.format(getBlockByHeight, height);
           log.info("chain id: {} get block height: {}",getChainId(),height);
            ...
            if (resultStr != null) {
                ...
                if (blockResult != null && blockResult.getCode() == 0) {
                            ...
                            public void run() {
                                ...
                                for (String tx : list) {
                                    try {
                                        ...
                                        String transactionUrl = nodeConfiguration.getOmniflixApi() + String.format(txs, hash);
                    ...
                    height++;
                    chainInfoService.updateHeight(nodeConfiguration.getOmniflixChainid(), height);
    ...
    public String getChainId() {
        return nodeConfiguration.getOmniflixChainid();
    }
}
